...
function TwitterRetweetModal(props) {
    ...
    const onSubmit = (values) => {
        ...
        dispatch({type: 'SET_ACTION_FILED_STATUS', name: 'TWITTER_RETWEET_ID_FINISHED'})
        onToggle()
    }

    return (<Modal isOpen={open} toggle={onToggle}>
        <ModalHeader className="modal-login-btn" style={{ borderBottom: 'none' }}>
            <div className="modal-login-btn">{t('twitter_retweet_modal.header')}</div>
        </ModalHeader>
        <ModalBody>
            <FinalForm
                onSubmit={onSubmit}
